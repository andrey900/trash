...
$arParams["ORDERS_PER_PAGE"] = (intval($arParams["ORDERS_PER_PAGE"]) <= 0 ? 20 : intval($arParams["ORDERS_PER_PAGE"]));
  // echo '<pre>'.print_r($arParams["ORDERS_PER_PAGE"] ,1).'</pre>'.__FILE__.' # '.__LINE__;
//Copy order
...
$minSumm = intval($_REQUEST['amount_from']);
$maxSumm = intval($_REQUEST['amount_to']);
if($minSumm > 0)  $arFilter['>=PRICE'] = $minSumm;
if($maxSumm > 0)  $arFilter['<=PRICE'] = $maxSumm;
