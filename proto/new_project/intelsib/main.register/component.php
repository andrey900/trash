...
if(!defined("B_PROLOG_INCLUDED")||B_PROLOG_INCLUDED!==true)die();
 ...
 //echo '<pre>'.print_r($arParams,1).'</pre>'.__FILE__.' # '.__LINE__;
 //echo '<pre>'.print_r($_POST,1).'</pre>'.__FILE__.' # '.__LINE__;


 if($arParams['LOGIN_EMAIL'] == 'Y')
 {
...
if(!is_array($arParams["USER_PROPERTY_REQUIRED_FIELDS"]))
	$arParams["USER_PROPERTY_REQUIRED_FIELDS"] = array();
...
if($arParams['CONFIRM_USER']=='Y')
	$arResult["USE_EMAIL_CONFIRMATION"] = 'Y';
elseif($arParams['CONFIRM_USER']=='N')
...
{  //
	...
	if($arParams['AUTO_PASS']=='Y')
	{
		$_REQUEST["REGISTER"]['PASSWORD'] = randString(8);
	...
	if( $arParams['CHEK_EMAIL'] != 'N')
		if(strlen($arResult["VALUES"]["EMAIL"]) > 0 && (COption::GetOptionString("main", "new_user_email_uniq_check", "N") === "Y" || $arParams['CHEK_EMAIL'] == 'Y'))
		{
	...
	if (is_array($arParams["USER_PROPERTY_REQUIRED_FIELDS"]) && count( $arParams["USER_PROPERTY_REQUIRED_FIELDS"]) > 0)
	{
		$arUserFieldsReq = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("USER", 0, LANGUAGE_ID);

		foreach ($arParams["USER_PROPERTY_REQUIRED_FIELDS"] as  $FIELD_NAME)
		{
 			if (!isset($FIELD_NAME,$_POST[$FIELD_NAME]) || trim($_POST[$FIELD_NAME] == ''))
					$arResult["ERRORS"][$FIELD_NAME] = str_replace("#FIELD_NAME#", '"'.$arUserFieldsReq[$FIELD_NAME]['EDIT_FORM_LABEL'].'"', GetMessage("REGISTER_FIELD_REQUIRED"));
		}
	}
    ...
    if(!is_array($arResult['VALUES']["GROUP_ID"]))
		$arResult['VALUES']["GROUP_ID"] = array();

	if(count($arParams['ADD_GROUP']) > 0)
		$arResult['VALUES']["GROUP_ID"] = array_merge($arResult['VALUES']["GROUP_ID"] , $arParams['ADD_GROUP']);

	//echo '<pre>'.print_r(array($def_group,$arParams['ADD_GROUP'],	$arResult['VALUES']["GROUP_ID"]),1).'</pre>'.__FILE__.' # '.__LINE__;
...
//return;
	...
	$GLOBALS["USER_FIELD_MANAGER"]->EditFormAddFields("USER", $arResult["VALUES"]);
		...
		{
			$user = new CUser();
			//     echo '<pre>'.print_r($arResult["VALUES"],1).'</pre>'.__FILE__.' # '.__LINE__;
		...
		if (intval($ID) > 0)
		{
			...
			if($bConfirmReq)
			{
                if($arParams['AUTO_PASS']=='Y')
                {
                   $arEventFields['USER_INFO'] = 'Ваши данные для входа в личный кабинет логин: '.$arResult['VALUES']['LOGIN'].' пароль: '.$arResult['VALUES']["PASSWORD"];
                }
