...
if($arParams['USE_JQUERY_FORMS'] == 'Y')  // подключени js файла обработчика форм
{
	if(!defined('addJqueryFormLoadHead'))
	{
	   define('addJqueryFormLoadHead',1);
       	$APPLICATION->AddHeadString('<script type="text/javascript" src="'.substr(dirname(__FILE__), strpos(__FILE__, '/bitrix/')).'/jquery.form.js"></script>',true);
    	$APPLICATION->AddHeadString('<script type="text/javascript" src="'.substr(dirname(__FILE__), strpos(__FILE__, '/bitrix/')).'/forms.function.js"></script>',true);
	}
}
...
if($_SERVER["REQUEST_METHOD"] != "POST")
{
	...
	if(count($arrSess) > 0)
	{
		...
		$_SESSION['feed_back_form'][$datas] = $arrSess;
...
if(($_SERVER["REQUEST_METHOD"] == "POST") &&( strlen($_POST["submit"]) > 0) && $ok)
{
	...
	{
        session_start();
		if(is_array($_SESSION['feed_back_form'][$_REQUEST['datas']]))
		{
			$_POST = array_merge($_POST,$_SESSION['feed_back_form'][$_REQUEST['datas']]);
