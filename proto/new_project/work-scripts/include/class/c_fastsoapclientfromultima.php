...
class FastSoapClientFromUltima extends SoapClientFromUltima {
    ...
    public function FGetAllGoodsActiveStatus()
    {
        $this->query->arGoodIds = array();
        $this->query->date = '';
        $this->_GetGoodsActiveStatusCSV();
        return $this->arResult;
    }

    public function FGetLastGoodsActiveStatus($date='')
    {
        $this->query->arGoodIds = array();
        $this->query->date = ($date)?$date:date('Y-m-d H:i:s');
        $this->_GetGoodsActiveStatusCSV();
    ...
    public function FGetGoodsActiveStatusFromId($id=array(0))
    {
        $this->query->arGoodIds = (is_array($id))?$id:array(0);
        $this->query->date = '';
        $this->_GetGoodsActiveStatusCSV();
    ...
    private function _GetGoodsActiveStatusCSV(){
        $this->arResult = $this->client->GetGoodsActiveStatusCSV($this->query->arGoodIds, 
                                                         $this->query->date,
                                                         43);
        return $this->_FUncover()->_clearQueryParam();
    }
