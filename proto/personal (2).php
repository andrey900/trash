...
class Personal{
	...
	/**
	 * Изменение инф о type пользователе
	 **/
	public function changePersonType(){
		try {
			$this->setParam(array('UF_SALE_PERSON_TYPE'));
			$this->BX_update(array('UF_SALE_PERSON_TYPE'=>$this->arRequest['UF_SALE_PERSON_TYPE']));
		} catch (Exception $e) {
    		$this->result->error = 'Возникла ошибка: '.  $e->getMessage(). "\n";
		}
	}
...
/*		else
	...
	private function BX_update($fields){
		...
		if( empty($curuser->LAST_ERROR) ){
			...
			$this->result->success =  "Персональные данные пользователя успешно изменены!";
		} else
			$this->result->error = $curuser->LAST_ERROR;
	}
...
echo json_encode($personal->result);
?>