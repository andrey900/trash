...
<?//php echo "<pre>"; print_r($Property); echo "</pre>"?>
        ...
        <?foreach($Property->GetValues() as $arVal):?>
	      	<div class="one-ch1">
	        	<label value="<?=$arVal['ID']?>">
	        	 	<b class="upper-case"><?=$arVal["NAME"]?> <span>(<?=$arVal["~COUNT"]?>)</span></b>
	        		<?$is_check = $Property->IsValueSelected($arVal['ID'])?'disabled="disabled" checked="checked"':"";?>
	          	<input <?/*if($arVal['CODE']):?> url="<?=$APPLICATION->GetCurPage()?><?=$arVal['CODE']?>/" <?endif;*/?> onchange="ReCalcCountInFilter(this)" <?=$is_check?> type="checkbox" name="<?=$arParams["FILTER_NAME"]?>[<?=$Property->GetID()?>][]" value="<?=$arVal['ID']?>" id="label_<?=$Property->GetID()?>_<?=$arVal['ID']?>" /> 
	          </label>
	        </div>
				<?endforeach;?>
 	    </div>
