...
//GTM google
/*
if($arResult['NAV_RESULT']->NavPageNomer>0 && $arResult['NAV_RESULT']->NavPageSize>0){
	$GTM_items = array();
	foreach($arResult['ITEMS'] as $key => $Item){
		$GTM_items[$arResult['NAV_RESULT']->NavPageSize*$arResult['NAV_RESULT']->NavPageNomer-$arResult['NAV_RESULT']->NavPageSize+$key] = $Item;
	}
	GTMDataCollector('impressions', $GTM_items);
}else
{
	GTMDataCollector('impressions', $arResult['ITEMS']);
}*/

//GTM
$listType = (isset($_REQUEST['filter']) && !empty($_REQUEST['filter']) )?'catalog-filters':'catalog';

if($arParams['GTM_CONTAINER'] && $arParams['GTM_LAYER']){
	if(strstr($arParams['GTM_CONTAINER'], ",")){	
		$exGTMContainer = explode(",", $arParams['GTM_CONTAINER']);
		foreach($exGTMContainer as $container){
			GTMNavPosition($container, $arResult['ITEMS'], $listType, $arResult['NAV_RESULT']);
			//GTMDataCollector($container, $arResult['ITEMS'], $arParams['GTM_LAYER']);
		}
	}else{
		GTMNavPosition($arParams['GTM_CONTAINER'], $arResult['ITEMS'], $listType, $arResult['NAV_RESULT']);
		//GTMDataCollector($arParams['GTM_CONTAINER'], $arResult['ITEMS'], $arParams['GTM_LAYER']);
	}
} else {
	GTMNavPosition('impressions', $arResult['ITEMS'], $listType, $arResult['NAV_RESULT']);
	//GTMDataCollector('impressions', $arResult['ITEMS']);
}
//end GTM
