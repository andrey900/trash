...
//p($arResult);
//GTM
$listType = (isset($_GET['filter']) && !empty($_GET['filter']) )?'catalog-filters':'catalog';
if($arParams['GTM_CONTAINER'] && $arParams['GTM_LAYER']){
	if(strstr($arParams['GTM_CONTAINER'], ",")){	
		...
		foreach($exGTMContainer as $container){
			GTMNavPosition($container, $arResult['ITEMS'], $arParams['GTM_LAYER'], $arResult['NAV_RESULT']);
			//GTMDataCollector($container, $arResult['ITEMS'], $arParams['GTM_LAYER']);
		}
	}else{
		GTMNavPosition($arParams['GTM_CONTAINER'], $arResult['ITEMS'], $arParams['GTM_LAYER'], $arResult['NAV_RESULT']);
