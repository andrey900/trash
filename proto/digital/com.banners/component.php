...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		if ($arParams["SET_BANNER_TYPE_FROM_THEME"]=="Y")
		{
			...
			$bannerType = strToLower($TEMPLATE_OPTIONS["COLOR_THEME"]["CURRENT_VALUE"]);
			$res = CIBlockElement::GetList(Array("SORT"=>"ASC"),  Array("IBLOCK_ID"=> $arParams["TYPE_BANNERS_IBLOCK_ID"], "CODE"=>$bannerType), false, false, Array("IBLOCK_ID", "ID", "CODE"));
			if($ar_res = $res->GetNext()) $bannerTypeID = strToLower($ar_res["ID"]);		
		} else { $bannerTypeID = $arParams["TYPE_BANNERS"]; }
		...
		$arFilter = array (
			...
			"IBLOCK_LID" => SITE_ID,
			"ACTIVE" => "Y",
			"PROPERTY_TYPE_BANNERS" => $bannerTypeID,
		);
