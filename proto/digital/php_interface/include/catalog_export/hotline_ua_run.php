...
if (empty($arRunErrors))
{
	...
	@fwrite($fp, "<price>\n");
        ...
        @fwrite($fp, "<date>".date('Y-m-d H:i')."</date>\n");

	@fwrite($fp, "<firmName>".$APPLICATION->ConvertCharset(htmlspecialcharsbx(COption::GetOptionString("main", "site_name", "")), LANG_CHARSET, 'windows-1251')."</firmName>\n");

	@fwrite($fp, "<firmId>1</firmId>\n");
        //@fwrite($fp, "<rate></rate>\n");
	...
	foreach ($arAvailGroups as &$value)
	{
		...
		$strTmpCat .= "<category>\n";
                $strTmpCat .= "<id>".$value["ID"]."</id>\n";
                if((int)$value["IBLOCK_SECTION_ID"] > 0) {
                    $strTmpCat .= "<parentId>".$value["IBLOCK_SECTION_ID"]."</parentId>\n";
                }
                $strTmpCat .= "<name>".yandex_text2xml($value["NAME"], true)."</name>\n";
                $strTmpCat .= "</category>\n";
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
					...
					//$strOfferYandex .= "<item id=\"".$arOfferItem["ID"]."\"".$str_TYPE." available=\"".$arOfferItem['YANDEX_AVAILABLE']."\">\n";
                                        ...
                                        $strOfferYandex .= "<id>".$arOfferItem["ID"]."</id>\n";
                                        $strOfferYandex .= $arItem['YANDEX_CATEGORY'];
                                        $strOfferYandex .= "<code>".$arOfferItem["CODE"]."</code>\n";
							...
							if (is_array($XML_DATA) && is_array($XML_DATA['XML_DATA']) && $XML_DATA['XML_DATA'][$key])
							{
								...
								if ('' != $strValue)
									$strOfferYandex .= $strValue."\n";
                                        ...
                                        $strOfferYandex .= "<url>http://".$ar_iblock['SERVER_NAME'].$arOfferItem["~DETAIL_PAGE_URL"]."</url>\n";
					...
					if (!empty($strFile) || !empty($arItem['YANDEX_PICT']))
					{
						$strOfferYandex .= "<image>".(!empty($strFile) ? $strFile : $arItem['YANDEX_PICT'])."</image>\n";
					}
                                        
					$strOfferYandex .= "<priceRUAH>".$minPrice."</priceRUAH>\n";
                                        
                                        if(!empty($minPriceCurrency)) {
                                            $strOfferYandex .= "<oldprice>".$minPriceCurrency."</oldprice>\n";
                                        } else {
                                            $strOfferYandex .= "<oldprice>".$minPrice."</oldprice>\n";
                                        }
                                        
                                        $stockInfo = $APPLICATION->ConvertCharset(htmlspecialcharsbx('На складе'), LANG_CHARSET, 'windows-1251');
                                        $guaranteeInfo = 12;
                                        
                                        $strOfferYandex .= "<stock>".$stockInfo."</stock>\n";
                                        $strOfferYandex .= "<guarantee>".$guaranteeInfo."</guarantee>\n";
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
				...
				$strOfferYandex = '';
				$strOfferYandex.= "<item id=\"".$arItem["ID"]."\"".$str_TYPE.$str_AVAILABLE.">\n";
	...
	{
		$strTmpCat .= "<category>\n";
                $strTmpCat .= "<id>".$intMaxSectionID."</id>\n";
                $strTmpCat .= "<name>".yandex_text2xml(GetMessage('YANDEX_ROOT_DIRECTORY'), true)."</name>\n";
                $strTmpCat .= "</category>\n";
	}
	@fwrite($fp, $strTmpCat);
	@fwrite($fp, "</categories>\n");
