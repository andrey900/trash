...
$arFilter = Array("IBLOCK_ID"=>"11", "ELEMENT_CNT" > 0, "CNT_ACTIVE" => "Y");
...
//get brands
if(!empty($PROP["BRANDS"]["VALUE"])){
    $arBrands = array();
    $arSelect = Array("ID", "NAME", "PROPERTY_BRAND");
    $arFilter = Array("IBLOCK_ID"=>"11", "SECTION_ID" => $PROP["SECTIONS"]["VALUE"]);
    $res = CIBlockElement::GetList(Array(), $arFilter, array("PROPERTY_BRAND"), false, $arSelect);
    $brands_ids = array();
    while($ob = $res->GetNext())
    {
        $brands_ids[] = $ob["PROPERTY_BRAND_VALUE"];
    }    
    
    //get brands information
    if(!empty($brands_ids)){
        $arSelect = Array("ID", "NAME");
        $arFilter = Array("IBLOCK_ID"=>"4", "ID" => $brands_ids);
        $res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
        while($ob = $res->GetNext()) 
        {
            $arBrands[$ob["ID"]] = $ob["NAME"];
        }          
    }
}
else{
...
if(!empty($PROP["SECTIONS"]["VALUE"])){
    $arElements = array();
    //found elements in current sections, selected brands
    $arSelect = Array("ID", "NAME", "PROPERTY_BRAND");
    $arFilter = Array("IBLOCK_ID"=>"11", "SECTION_ID" => $PROP["SECTIONS"]["VALUE"], "PROPERTY_BRAND" => $PROP["BRANDS"]["VALUE"]);
    $res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
    while($ob = $res->GetNext())
    {
        $arElements[] = $ob;
...
$tabControl->AddSection("IBLOCK_ELEMENT_LINKS", 'Связанные элементы');
$tabControl->BeginCustomField("PROPERTY_CUSTOM", "SELECT_PARAMS", "Y");
?>
<tr>
    <td style="text-align: center; width: 49%;">
        <label for="section_select">Выберите разделы</label>
        <select multiple size = "4" id="section_select" name="PROP[113][]">
        <?foreach($arSections as $sect):?>
            <option value="<?=$sect["ID"]?>"<?if(in_array($sect["ID"], $PROP["SECTIONS"]["VALUE"])):?> selected="selected"<?endif;?>><?=$sect["NAME"]?></option>
        <?endforeach;?>
        </select>
        <input class="button" id="section_select_button" type="submit" value="Найти бренды"> 
    </td>
    <td style="text-align: center; width: 49%;">
        <label for="brands_select">Выберите бренды</label>
        <select multiple size = "4"<?if(empty($arBrands)):?> disabled="disabled"<?endif;?> id="brands_select" name="PROP[114][]">
        <?if(!empty($arBrands)):?>
            <?foreach($arBrands as $k => $brand):?>
                <option value="<?=$k?>"<?if(in_array($k, $PROP["BRANDS"]["VALUE"])):?> selected="selected"<?endif;?>><?=$brand?></option>
            <?endforeach;?>
        <?else:?>
        <?endif;?>    
        </select>
        <input class="button" id="brand_select_button_news" type="submit" value="Подобрать элементы"<?if(empty($arBrands)):?> disabled="disabled"<?endif;?>>
    </td>
</tr>
<tr>
    <td colspan="2">
        <input class="button" id="elements_select_button" type="submit" value="Выбрать все элементы">
        <ul class="action_elements_list">
        <?foreach($arElements as $elem):?>
            <li><input name='PROP[109][]' type='checkbox' value='<?=$elem["ID"]?>'<?if(in_array($elem["ID"], $PROP["GOODS"]["VALUE"])):?> checked="checked"<?endif;?>><label for='<?=$elem["ID"]?>'><?=$elem["NAME"]?></label></li>
        <?endforeach;?>
        </ul>
    </td>
</tr>
<?
$tabControl->EndCustomField("PROPERTY_CUSTOM", "");
if(!empty($PROP)):
    ...
    $custom_props = array("GOODS", "SECTIONS", "BRANDS");
    foreach($PROP as $prop_code=>$prop_fields):
    if (in_array($prop_code, $custom_props)) {
